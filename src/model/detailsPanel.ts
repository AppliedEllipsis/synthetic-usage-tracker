...
// Lightweight rendering panel for Models and Tool Quotas in details view
export class DetailsPanel {
  private _panel: vscode.WebviewPanel | undefined;
  private _models: ModelInfo[] = [];
  private _toolQuotas: ToolQuotaInfo[] = [];

  constructor(private context: vscode.ExtensionContext) {}

  public setData(usageInfo: UsageInfo, models?: ModelInfo[], toolQuotas?: ToolQuotaInfo[]): void {
    if (models) this._models = models;
    if (toolQuotas) this._toolQuotas = toolQuotas;
    this._ensurePanel();
    this._updatePanel(usageInfo);
  }

  private _ensurePanel(): void {
    if (this._panel) return;
    this._panel = vscode.window.createWebviewPanel('syntheticDetails', 'Synthetic Usage Details', vscode.ViewColumn.Active, {
      enableScripts: true
    });
  }

  private _updatePanel(usageInfo: UsageInfo): void {
    if (!this._panel) return;
    this._panel.webview.html = this._getHtmlForWebview(usageInfo);
  }

  private _getHtmlForWebview(usageInfo: UsageInfo): string {
    const modelsHtml = this._renderModelsSection(this._models);
    const quotasHtml = this._renderToolQuotasSection(this._toolQuotas);
    const html = `<!DOCTYPE html>
      ...
      <head>
        ...
        <meta name="viewport" content="width=device-width, initial-scale=1.0" />
        <style>
          body { font-family: -apple-system, BlinkMacSystemFont, Segoe UI, Roboto; margin: 0; padding: 12px; }
  ...
  private _renderModelsSection(models: ModelInfo[]): string {
    if (!models || models.length === 0) return '';
    const rows = models.map(m => `
      <div class="card">
        <div><strong>${m.name}</strong> ${m.description ? `<span style="color:#666">- ${m.description}</span>` : ''}</div>
        <div style="margin-top:6px">Pricing: Input ${m.pricing?.input ?? '—'}, Output ${m.pricing?.output ?? '—'}</div>
        <div>Context Window: ${m.contextWindow ?? ''}</div>
      </div>`).join('');
    return `
  ...
  private _renderToolQuotasSection(quotas: ToolQuotaInfo[]): string {
    if (!quotas || quotas.length === 0) return '';
    const rows = quotas.map(q => `
      <div class="card">
        <div><strong>${q.toolName}</strong></div>
        <div>Usage: ${q.usage}/${q.limit} (${q.percentage ?? Math.round((q.usage / q.limit) * 100) }%)</div>
        <progress value="${q.usage}" max="${q.limit}"></progress>
      </div>`).join('');
    return `
      <div class="section" id="quotas-section">
        <h2>Tool Quotas</h2>
        ${rows}
      </div>`;
  }
...
// Type definitions used for rendering in the details panel
...
export interface ToolQuotaInfo {
  toolName: string;
  usage: number;
  limit: number;
  percentage?: number;
...
// Minimal UsageInfo type placeholder (should align with actual type in project)
export interface UsageInfo {
  // include fields as needed by calling code; for HTML rendering only, minimal stub
}
